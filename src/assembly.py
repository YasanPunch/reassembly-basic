...
def check_overlap(mesh1, mesh2, voxel_size, max_overlap_factor=0.5): # voxel_size IS used in the manual fallback or could be in future point-based checks
    ...
    if hasattr(aabb1, 'get_intersection'):
        ...
        print("    DEBUG assembly.py: aabb1.get_intersection not found by hasattr, calculating manually.") # Keep this debug!
        ...
        if (intersect_min_x < intersect_max_x and
            intersect_min_y < intersect_max_y and
            intersect_min_z < intersect_max_z):
            vol_intersection = (intersect_max_x - intersect_min_x) * \
...
class Assembler:
    ...
    def greedy_assembly(self):
        ...
        print(f"Starting assembly with seed fragment: {self.fragments_data[seed_idx]['name']} (idx {seed_idx})")
        self.is_fragment_placed[seed_idx] = True
        current_assembly_indices = {seed_idx}
        current_assembly_meshes = [self._get_transformed_mesh(seed_idx)] # Store meshes for overlap checks
        ...
        while num_placed < self.num_fragments:
            ...
            for match_info in self.pairwise_matches:
                ...
                if self.is_fragment_placed[t_idx] and not self.is_fragment_placed[s_idx]:
                    if match_info['score'] > best_candidate_score:
                        ...
                        proposed_transform_s = np.dot(self.fragment_transforms[t_idx], match_info['transformation'])
                        ...
                        for placed_mesh in current_assembly_meshes:
                            if not check_overlap(temp_s_mesh, placed_mesh, 
                ...
                # Proposed transform for t_idx: T_world_source * T_source_target
                    ...
                    if match_info['score'] > best_candidate_score:
                        ...
                        try:
                            inv_transform_s_to_t = np.linalg.inv(match_info['transformation'])
                        except np.linalg.LinAlgError:
                            # print(f"Warning: Could not invert transformation for match {s_idx}->{t_idx}. Skipping.")
                            continue
                        
                        proposed_transform_t = np.dot(self.fragment_transforms[s_idx], inv_transform_s_to_t)
                        ...
                        overlap_ok = True
                        for placed_mesh in current_assembly_meshes:
                            if not check_overlap(temp_t_mesh, placed_mesh,
                                                 self.params['voxel_downsample_size'],
                                                 self.params.get('max_assembly_overlap_factor', 0.6)):
                                overlap_ok = False
            ...
            if best_candidate_match:
                ...
                current_assembly_meshes.append(self._get_transformed_mesh(newly_placed_idx)) # Add its transformed version
                num_placed += 1
                print(f"  Placed fragment: {self.fragments_data[newly_placed_idx]['name']} (idx {newly_placed_idx}) "
