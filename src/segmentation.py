...
def region_growing_segmentation(tri_mesh, params):
    ...
    # Ensure regions are returned in the correct order with consistent indexing
    # The regions should be returned in the same order they appear in the list
    # so that when we assign indices later, they match the validation expectations
...
def visualize_merged_regions(merged_clusters, all_triangles, mesh_vis):
    """
    Visualize merged regions for debugging purposes.

    Args:
        merged_clusters (list): List of tuples containing (cluster_faces, merged_idxs)
        all_triangles (np.ndarray): All triangle indices from the mesh
        mesh_vis (o3d.geometry.TriangleMesh): Base mesh for visualization
    """
...
def visualize_saved_segmentation_results(
    saved_regions, saved_merged_surfaces, mesh_vis
):
    """
    Visualize saved segmentation results for debugging purposes.

    Args:
        saved_regions (list): List of region face indices arrays
        saved_merged_surfaces (list): List of Open3D meshes (merged fracture surfaces)
        mesh_vis (o3d.geometry.TriangleMesh): Base mesh for visualization
    """
    print(f"\n    Visualizing {len(saved_merged_surfaces)} saved merged surfaces...")
    for idx, surface_mesh in enumerate(saved_merged_surfaces):
        if surface_mesh is None or not surface_mesh.has_triangles():
            continue

        # Create a copy for visualization
        vis_mesh = copy.deepcopy(surface_mesh)
        color = get_color(idx, len(saved_merged_surfaces))
        vis_mesh.paint_uniform_color(color)

        print(
            f"    [DEBUG] Saved surface {idx}: triangles: {len(surface_mesh.triangles)}, "
            f"vertices: {len(surface_mesh.vertices)}, color: {color}"
        )
        ...
        o3d.visualization.draw_geometries(
            [wireframe, vis_mesh],
            window_name=f"[DEBUG] Saved Surface {idx} (Color {idx})",
        )
...
def extract_fracture_surface_mesh(o3d_mesh_fragment, fragment_name="Unnamed", params=None):
        ...
        if (
            ...
            if validate_segmentation_results(
                ...
                if params.get("visualize_merged_regions", False):
                    print(f"    Visualizing saved segmentation results...")
                    mesh_vis = copy.deepcopy(
                        o3d_mesh_fragment
                    )  # Always create mesh_vis for later use
                    visualize_saved_segmentation_results(
                        saved_regions, saved_merged_surfaces, mesh_vis
                    )
    ...
    # Update indices to match their new positions after sorting
    for i, props in enumerate(region_properties):
        props["index"] = i

    # Update regions list to match the sorted order of region_properties
    regions = [props["faces"] for props in region_properties]
    ...
    if params.get("visualize_merged_regions", False):
        visualize_merged_regions(merged_clusters, all_triangles, mesh_vis)
