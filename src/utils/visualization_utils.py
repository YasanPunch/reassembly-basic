    ...
    if fragments_data_map:
        ...
        for name, orig_idx_from_map in fragments_data_map.items():
            ...
            if num_unique_frags_for_coloring > 0:
                ...
                original_index_to_color[orig_idx_from_map] = get_color(
                    color_map_idx, num_unique_frags_for_coloring
                )
    ...
    for i, entry in enumerate(log_data):
        ...
        if orig_idx != -1 and orig_idx in original_index_to_color:
            ...
            if orig_idx not in original_index_to_color: # Assign a new color
                ...
                original_index_to_color[orig_idx] = get_color(
                    color_assign_idx, num_unique_frags_for_coloring or 5
                )  # Default to 5 unique colors if num_unique not set
            frag_color = original_index_to_color[orig_idx]
        ...
        if geometries_to_draw:
            ...
            if valid_geoms:
                ...
                print("    No valid geometries to draw for this step.")
        else:
            print("    No geometries to draw for this step.")
