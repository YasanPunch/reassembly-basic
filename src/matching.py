...
from alignment import align_fragments_pcd
...
def find_pairwise_matches(fragments_data, params):
    ...
    for i, j in combinations(range(num_fragments), 2):
        ...
        print(
            f"  Attempting to match: {frag_i_data['name']} (idx {i}) <-> {frag_j_data['name']} (idx {j})"
        )
        ...
        if (
            ...
            or source_fpfh is None
            or target_fpfh is None
        ):
            print(f"    Skipping pair ({i}, {j}) due to missing PCD/features.")
            continue
        if (
            not source_pcd.has_points()
            or not target_pcd.has_points()
            or source_fpfh.num() == 0
            or target_fpfh.num() == 0
        ):
        ...
        if transform_j_to_i is not None and fitness_ji >= params.get(
            "min_match_score", 0.6
        ):
            potential_matches.append(
                {
                    "source_idx": j,
                    "target_idx": i,  # j is transformed to align with i
            ...
            print(
                f"    Match found ({j} to {i}): Score={fitness_ji:.3f}, RMSE={rmse_ji:.3f}"
            )

            # Visualize the alignment
            source_pcd_transformed = source_pcd.transform(transform_j_to_i)
            target_pcd.paint_uniform_color([1, 0, 0])  # Red for target
            source_pcd_transformed.paint_uniform_color(
                [0, 1, 0]
            )  # Green for transformed source
            o3d.visualization.draw_geometries(
                [target_pcd, source_pcd, source_pcd_transformed],
                window_name=f"Match: {frag_j_data['name']} to {frag_i_data['name']}",
            )
        ...
        if transform_i_to_j is not None and fitness_ij >= params.get(
            "min_match_score", 0.6
        ):
            potential_matches.append(
                {
                    "source_idx": i,
                    "target_idx": j,  # i is transformed to align with j
                    "transformation": transform_i_to_j,
                    "score": fitness_ij,
                    "rmse": rmse_ij,
                    "source_name": frag_i_data["name"],
                    "target_name": frag_j_data["name"],
                }
            )
            print(
                f"    Match found ({i} to {j}): Score={fitness_ij:.3f}, RMSE={rmse_ij:.3f}"
            )

            # Visualize the alignment
            source_pcd_transformed = source_pcd.transform(transform_i_to_j)
            target_pcd.paint_uniform_color([1, 0, 0])  # Red for target
            source_pcd_transformed.paint_uniform_color(
                [0, 1, 0]
            )  # Green for transformed source
...
"""
    ...
    if len(processed_fragments_data) >= 2:
        ...
        for match in pairwise_matches:
            print(
                f"  {match['source_name']} (idx {match['source_idx']}) -> {match['target_name']} (idx {match['target_idx']}) "
